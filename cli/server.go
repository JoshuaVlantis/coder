...
func ReadAIBridgeProvidersFromEnv(logger slog.Logger, environ []string) ([]codersdk.AIBridgeProviderConfig, error) {
	providers, err := readAIBridgeProvidersForPrefix(logger, environ, "CODER_AIBRIDGE_PROVIDER_")
	if err != nil {
		return nil, err
	}
	gatewayProviders, err := readAIBridgeProvidersForPrefix(logger, environ, "CODER_AI_GATEWAY_PROVIDER_")
	if err != nil {
		return nil, err
	}
...
// readAIBridgeProvidersForPrefix parses provider env vars under a single
// indexed prefix (e.g. CODER_AI_GATEWAY_PROVIDER_) into a slice of
// AIBridgeProviderConfig. Per-field syntax errors and unknown keys are
// reported using the original env var name so the prefix stays visible
// to the operator.
func readAIBridgeProvidersForPrefix(logger slog.Logger, environ []string, prefix string) ([]codersdk.AIBridgeProviderConfig, error) {
