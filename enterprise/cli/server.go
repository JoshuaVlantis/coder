...
func (r *RootCmd) Server(_ func()) *serpent.Command {
	cmd := r.RootCmd.Server(func(ctx context.Context, options *agplcoderd.Options) (*agplcoderd.API, io.Closer, error) {
		...
		if bridgeEnabled || proxyEnabled {
			...
			providers, err := loadProvidersFromDB(ctx, options.Database, options.DeploymentValues.AI.BridgeConfig)
			if err != nil {
				return nil, nil, xerrors.Errorf("load aibridge providers from db: %w", err)
			}
			...
			if aibridgeDaemon != nil {
				...
				unsub, err := options.Pubsub.Subscribe(coderd.AIBridgeProvidersChangedChannel, func(notifyCtx context.Context, _ []byte) {
					newProviders, err := loadProvidersFromDB(notifyCtx, options.Database, options.DeploymentValues.AI.BridgeConfig)
					if err != nil {
