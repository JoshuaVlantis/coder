...
describe("appendTextBlock", () => {
	...
	it("attaches reasoning timestamps to a new thinking block", () => {
		const result = appendTextBlock([], "thinking", "pondering", {
	...
	it("merges thinking blocks keeping earliest startedAt and latest completedAt", () => {
		...
		const result = appendTextBlock(blocks, "thinking", "second", {
			startedAt: "2026-04-21T00:00:00.000Z",
	...
	it("locks in completedAt from a thinking marker with empty text", () => {
		// Mirrors the chatloop end-of-reasoning marker: empty text
		// but new completedAt that should still be applied to the
		...
		const result = appendTextBlock(blocks, "thinking", "", {
			completedAt: "2026-04-21T00:00:02.000Z",
		});
		expect(result).toEqual([
			{
	...
	it("ignores empty text without timestamps even on thinking blocks", () => {
		const blocks: RenderBlock[] = [{ type: "thinking", text: "exists" }];
